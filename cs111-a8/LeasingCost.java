public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        for(int i = 0; i < description.length() - 4; i++){
            if(description.substring(i, i+4).equals("name"))
                name = description.substring(i+5);
        }
        name = name.substring(0, name.indexOf(";"));

        String type = "";
        for(int i = 0; i < description.length() - 4; i++){
            if(description.substring(i, i+4).equals("type"))
                type = description.substring(i+5);
        }
        int typeNum;
        if(type.substring(0, type.indexOf(";")).equals("electric"))
            typeNum = 2;
        else
            typeNum = 1;

        
        double chargerPrice = 0;
            if(typeNum == 2){
        String charger = "";
        for(int i = 0; i < description.length() - 7; i++){
            if(description.substring(i, i+7).equals("charger"))
                charger = description.substring(i+8);
        }
        chargerPrice = Double.parseDouble(charger.substring(0, charger.indexOf(";")));
        }
       
        
        String usage = "";
        for(int i = 0; i < description.length() - 9; i++){
            if(description.substring(i, i+9).equals("mile/unit"))
                usage = description.substring(i+10);
        }
        double usageMiles = Double.parseDouble(usage.substring(0, usage.indexOf(";")));

        Fuel fuel;
        if(typeNum == 1)
            fuel = new Fuel(usageMiles);
        else
            fuel = new Fuel(usageMiles, chargerPrice);
        
        String due = "";
        for(int i = 0; i < description.length() - 3; i++)
            if(description.substring(i, i+3).equals("due"))
                due = description.substring(i+4);
        due = due.substring(0, due.indexOf(";"));
        double dollarsDue = Double.parseDouble(due);

        String months = "";
        for(int i = 0; i < description.length() - 6; i++)
            if(description.substring(i, i+6).equals("length"))
                months = description.substring(i+7);
        months = months.substring(0, months.indexOf(";"));
        int length = Integer.parseInt(months);

        String cost = "";
        for(int i = 0; i < description.length() - 7; i++)
            if(description.substring(i, i+7).equals("monthly"))
                cost = description.substring(i+8);
        cost = cost.substring(0, cost.indexOf(";"));
        double monthlyCost = Double.parseDouble(cost);

        String allowed = "";
        for(int i = 0; i < description.length() - 9; i++)
            if(description.substring(i, i+9).equals("allowance"))
                allowed = description.substring(i+10);
        allowed = allowed.substring(0, allowed.indexOf(";"));
        int mileageAllowance = Integer.parseInt(allowed);

        Lease lease = new Lease(dollarsDue, length, monthlyCost, mileageAllowance);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        for(int i = 0; i < vehicles.length; i++){
            if(vehicles[i].getFuel().getType() == 1){
                ...
                vehicles[i].setCO2Emission(computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), Fuel.CO2EMITTED_GASCOMBUSTION));
                vehicles[i].setTotalCost(computeFuelCost(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), gasPrice) +
                                        computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost()));
            }
            else{
                ...
                vehicles[i].setCO2Emission(computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION));
                vehicles[i].setTotalCost(vehicles[i].getFuel().getCharger() + computeFuelCost(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), electricityPrice) +
                                        computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost()));
            }
